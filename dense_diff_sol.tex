...
\[ f_{U_k}(x)=n \cdot (C_{n-1})^{k-1} \cdot (frac{x-a}{b-a})^{k-1} \cdot (1-frac{x-a}{b-a})^{n-k} \cdot \frac{1}{b-a} \text{ , если }x \in [a; b]\]
}
...
\solution{
    Пусть искомое событие $A = \{ X_k\in [x; x+dx], X_m\in [y; y + dy]\}$

Искомое событие произошло, если одновременно:

\begin{enumerate}
    \item Какое-то одно наблюдение из n попало в $[x; x+dx]$ (с вероятностью $f_X(x)dx + o(dx)$)
    \item Какое-то одно наблюдение из (n-1) попало в $[y; y+dy]$ (c вероятностью $f_X(y)dy + o(dy)$)
    \item Какие-то (k–1) из оставшихся (n-1) левее $X_k$ с вероятностью $(F_X(x))^{k-1}$
    \item Какие-то (m–k–1) из оставшихся (n-k-1) попали в $[X_k; X_m]$ с вероятностью $(F_X(y)- F_X(x))^{m-k-1}$
    \item Оставшиеся (n–1) правее $X_m$ с вероятностью $(1-F_X(y))^{n-1}$
\end{enumerate}

Тогда:
\begin{multline*}
    f_{X_k\cm X_m}(x\cm y)dx  dy = n(n - 1)\cdot (C_{n-1})^{k-1} \cdot (f_X(x)+o(dx))\cdot (f_X(y)+o(dy))\cdot (F_X(x))^{k-1}\cdot \\ \cdot (F_X(y) - F_X(x))^{m-k-1} \cdot (1- F_X(y))\cdot (n-1) + o(dx) + o(dy)
\end{multline*}


Тогда для равномерного распределения:
\begin{multline*}
f_{U_k\cm U_m}(x, y)dx dy = n(n - 1)\cdot (C_{n-1})^{k-1} \cdot \left( \frac{x-a}{b-a} \right)^{k-1}\cdot \left( \frac{y-a}{b-a}- \frac{x-a}{b-a}\right)^{m-k-1}\cdot \\ \cdot \left(1- \frac{y-a}{b-a}\right)\cdot (n-1)\cdot \left(\frac{1}{b-a}\right)^2\text{, если } x \in [a; b] 
\end{multline*}
}
...
\solution{
    Так как функция плотности произвольной абсциссы имеет вид $f_X (x)dx = \frac{2\cdot \sqrt{R^2-x^2}}{\pii\cdot R^2}dx+o(dx)$, если $x \in [-R; R]$, а плотность $k$-й порядковой статистики:

    \[f_{X_k}(x)=n(f_X(x)dx + o(dx)) \cdot  (C_{n-k})^{k-1}\cdot (F_X(x))^{k-1}\cdot (1-F_X(x))^{n-k}\cdot 1 + o(dx)\,\] то:

\begin{multline*}
    f_{X_k}(x)=n\cdot \left(\frac{2\cdot \sqrt{R^2-x^2}}{\pii\cdot R^2}\right) \cdot (C_{n-k})^{k-1}\cdot \left(\frac{x\cdot \sqrt{R^2-x^2}+R^2\cdot \arctg{\frac{x}{\sqrt{R^2-x^2}}}}{\pii\cdot R^2}\right)^{k-1}\cdot \\ \cdot \left(1-\frac{x\cdot \sqrt{R^2-x^2}+R^2\cdot \arctg{\frac{x}{\sqrt{R^2-x^2}}}}{\pii\cdot R^2}\right)^{n-k}
\end{multline*}

    Аналогично для ординаты.
}
...
\solution{
    Так как функция плотности произвольной абсциссы имеет вид $f_X (x)dx = 2\cdot (1-x)$, если $x \in [0; 1]$, а плотность $k$-й порядковой статистики:
    \[f_{X_k}(x)=n(f_X(x)dx + o(dx)) \cdot  (C_{n-k})^{k-1}\cdot (F_X(x))^{k-1}\cdot (1-F_X(x))^{n-k}\cdot 1 + o(dx)\,\] то:
    \[ f_{X_k}(x)=n\cdot (2\cdot (1-x))\cdot (C_{n-k})^{k-1}\cdot (2\cdot x-x^2)^{k-1}\cdot (1-(2\cdot x-x^2))^{n-k} \]
}
...
\solution{
    Так как функция плотности произвольной абсциссы имеет вид $f_X (x)dx = \frac{ln(1+x)}{2ln(2)-1}$, если $x \in [0; 1]$, а плотность $k$-й порядковой статистики:
    \[f_{X_k}(x)=n(f_X(x)dx + o(dx)) \cdot  (C_{n-k})^{k-1}\cdot (F_X(x))^{k-1}\cdot (1-F_X(x))^{n-k}\cdot 1 + o(dx)\,\] то:
    \begin{multline*}
        f_{X_k}(x)=n\cdot \left(\frac{ln(1+x)}{2\cdot ln(2)-1}\right)\cdot (C_{n-k})^{k-1}\cdot \left(\frac{(1 + x)\cdot ln(1 + x) - x}{2\cdot ln(2)-1}\right)^{k-1}\cdot \\ \cdot \left(1-(\frac{(1 + x)\cdot ln(1 + x) – x}{2ln(2)-1})\right)^{n-k} \text{, если } x \in [0; 1]
    \end{multline*}

    Для ординаты:
    \[f_{Y_k}(y)=n\cdot \left(\frac{2-e^{y}}{ 2\cdot ln(2)-1}\right)\cdot C_{n-k}^{k-1}\cdot \left(\frac{2\cdot y-e^{y}}{2\cdot ln(2)-1}\right)^{k-1}\cdot \left(1-\frac{2\cdot y-e^{y}}{2ln(2)-1}\right)^{n-k} \text{, если } x \in [0; ln(2)] \]
}
\end{document}
