...
Тогда для равномерного распределения:
\begin{multline*}
f_{U_k, U_m}(x, y)dx ^ dy = n(n - 1)\cdot (C_{n-1})^{k-1} \cdot \left( \frac{x-a}{b-a} \right)^{k-1}\cdot \left( \frac{y-a}{b-a}- \frac{x-a}{b-a}\right)^{m-k-1}\cdot \\ \cdot \left(1- \frac{y-a}{b-a}\right)\cdot (n-1)\cdot \left(\frac{1}{b-a}\right)^2\text{, если } x \in [a; b] 
\end{multline*}
}
